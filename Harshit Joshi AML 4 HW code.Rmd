...
```{r}
# loading data as dataframe from csv
Essays_main <-data.frame( read.csv('HW4-data-fedPapers85(1).csv'))
#exploring
table(
  Essays_main$author)

#Since we only need to find whether diputed essays belong to hamilton or Madisson, I will remove authors with name HM and Jays, so that we will only have Hamilton, Madisson and Disputed as authors in our dataframe

Essays_main <- Essays_main[!(Essays_main$author == 'HM' | Essays_main$author == 'Jay') , ]

table(Essays_main$author)
```


```{r}
#our dataset contains 77 observation of 72 variables.
str(Essays_main)
```

```{r}
#removing null values if we have them
Essays_main = na.omit(Essays_main)
```

```{r}
#In order to run kmeans, I need data in form of matrix, therefore I need to convert my data a matrix. But for this I will need to remove catagorical values from my dataset. Therefore I removed the author and filename column and stored it in new variable called essay_main.unlabeled
Essays_main.unlabeled = Essays_main[, which(!names(Essays_main) %in% c('author', 'filename'))]
head(Essays_main.unlabeled)
```

```{r}
#distance = factoextra::get_dist(Essays_main.unlabeled) # default method is euclidean

# visualization of the distance matrix
#factoextra::fviz_dist(distance,
     #                 gradient = list(low = "#00AFBB", mid = "white", high = "#FC4E07")) 
```
```{r}
Essays_main.unlabeled = scale(Essays_main.unlabeled)
head(Essays_main.unlabeled)

typeof(Essays_main.unlabeled)
```

```{r}

#Now I put my unlabeled data in kmeans() and analyze the values for each cluster, each word:
model.r = kmeans(Essays_main.unlabeled,
                 centers = 2, nstart = 25
                 )
model.r
```
...
```{r}
# As per problem statement, I analyzed the centroids of each cluster to find which words are more useful for formation of clusters:
model.r$centers
#It looks like every word has been helpful in building cluster because the values for each word are on either side of zero for each cluster. 
#If we go in more details, words like there, upon, to, on have highest difference between the 2 clusters compare to other words. It can be said that these words are more helpful for us in clustering.
```
```{r}
#Now we assign each row (each essay) with its respective cluster
cluster.assignment = data.frame(Essays_main, model.r$cluster) |>
  relocate(model.r.cluster, .after = author)
glimpse(cluster.assignment)
```
```{r}
#plotting clusters:
fviz_cluster(model.r, data = Essays_main.unlabeled)
```
```{r}
#This is not needed as we already know what is number of cluster needed. so we know that k must be 2
set.seed(123)
fviz_nbclust(Essays_main.unlabeled, kmeans, method = "wss", k.max = 9)
```
```{r}
#not needed
set.seed(123)
fviz_nbclust(Essays_main.unlabeled, kmeans, method = "silhouette")
```
```{r}
#not needed
set.seed(123)
gap.stat = clusGap(Essays_main.unlabeled, FUN = kmeans,
                   K.max = 8, B = 50)
...
final.res = kmeans(Essays_main.unlabeled, 2)

fviz_cluster(final.res, data = Essays_main.unlabeled)
...
```
```{r}
Essays_main$cluster = final.res$cluster 
Essays_main |>
  count(cluster) |> 
  left_join(
    Essays_main |> 
      select(cluster, author) |> 
      group_by(cluster) |>
      mutate(author_name = str_c(author, collapse = ', ')) |>
      select(-author) |> 
      ungroup() |> 
      distinct()
  )
...
```{r}
# I got this code from (http://pcwww.liv.ac.uk/~william/R/crosstab.r) that will create crosstable function of two columns with unique values:
...
```
```{r}
#using crosstab funcation to how many dispt essays are in each cluster
crosstab(cluster.assignment, row.vars = "model.r.cluster", col.vars = "author", type = "f")

#looks like cluster 1 is Hamilton cluster with 0 cluster and cluster 2 is Madison cluster with all 11 dispt essays. Therefore it can be said that Madison wrote 11 disputed essays.
```
